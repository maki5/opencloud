...
// UserResolver resolves a username to a user identity (via the CS3 gateway).
type UserResolver interface {
	GetUserByClaim(ctx context.Context, claim, value string, auth string) (*userv1beta1.User, error)
}
...
// WithUserResolver sets the resolver used to look up users by claim (username).
...
func (w *ThumbnailWorkflow) Execute(ctx context.Context, tr *requests.ThumbnailRequest, auth string, logger log.Logger) (data []byte, ext string, aIgnored bool, err error) {
	...
	case tr.Identifier == "":
		...
		ref = &providerv1beta1.Reference{Path: filepath.Join(templates.WithUser(user, w.webdavNS), tr.Filepath)}
	case strings.Contains(tr.Identifier, "$"):
		parsedRef, perr := storagespace.ParseReference(filepath.Join(tr.Identifier, tr.Filepath))
		if perr != nil {
			return nil, "", false, fmt.Errorf("parse space reference: %w", perr)
		}
		ref = &parsedRef
	default:
		// The identifier is a username (dav/files/{username}); resolve it to the
		// user's home path like the legacy webdav handler did.
		user, err := w.resolveUserByUsername(ctx, tr.Identifier, auth)
		if err != nil {
			return nil, "", false, fmt.Errorf("resolve user: %w", err)
		}
		ref = &providerv1beta1.Reference{Path: filepath.Join(templates.WithUser(user, w.webdavNS), tr.Filepath)}
...
func (w *ThumbnailWorkflow) ExecutePublic(ctx context.Context, tr *requests.ThumbnailRequest, auth string, logger log.Logger) (data []byte, ext string, aIgnored bool, err error) {
	ref := &providerv1beta1.Reference{Path: filepath.Join("/public", tr.PublicLinkToken, tr.Filepath)}
	data, ext, aIgnored, err = w.generate(ctx, ref, auth, tr, logger)
	return data, ext, aIgnored, err
}
...
//   - thumbnail         -> fill (square tiles; legacy "thumbnail" processor crops)
//   - (none), gif       -> stretch (resize for gifs)
//   - (none), non-gif   -> fill (default = thumbnail = fill)
//
// The second return value reports whether the legacy "a" flag was ignored because
// an explicit processor overrode it with a different behavior, so the caller can
// tell the client via a response header.
...
func (w *ThumbnailWorkflow) resolveUserByUsername(ctx context.Context, username, auth string) (*userv1beta1.User, error) {
	if w.userResolver != nil {
		return w.userResolver.GetUserByClaim(ctx, "username", username, auth)
	}
	...
	userRes, err := client.GetUserByClaim(ctx, &userv1beta1.GetUserByClaimRequest{Claim: "username", Value: username})
	if err != nil {
...
// gatewayUserResolver implements UserResolver using the CS3 gateway.
type gatewayUserResolver struct {
	selector gatewaySelector
}

func (g *gatewayUserResolver) GetUserByClaim(ctx context.Context, claim, value, auth string) (*userv1beta1.User, error) {
	client, err := g.selector.Next()
	if err != nil {
		return nil, fmt.Errorf("get gateway client: %w", err)
	}
